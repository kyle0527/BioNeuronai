...
class MockBinanceConnector:
    ...
    def __init__(
        ...
        speed_multiplier: float = 0.0,  # 0 = 無延遲
        # 以下為偽裝用參數（與真實 Connector 接口一致）
        api_key: str = "",
        api_secret: str = "",
        testnet: bool = True,
    ):
        ...
        Args:
            ...
            api_key: (偽裝用) API Key
            api_secret: (偽裝用) API Secret
            testnet: (偽裝用) 是否測試網
        """
        # 偽裝參數（讓 TradingEngine 以為是真實連接）
        self.api_key = api_key or "MOCK_API_KEY_FOR_BACKTEST"
        self.api_secret = api_secret or "MOCK_SECRET_FOR_BACKTEST"
        ...
        logger.info("=" * 60)
        logger.info(f"模式: 歷史數據回放 (AI 無法區分真假)")
