...
from bioneuronai.common.detection_config import SQLiConfig
...
class ProductionUnionSQLiEngine(DetectionEngineProtocol):
    ...
    def _analyze_union_response(
        ...
        if re.search(number_pattern, response_text) and not re.search(
            number_pattern, baseline_text
        ):
    ...
    def _create_finding(
        ...
        return FindingPayload(
            ...
            target=FindingTarget(
                url=str(target.url), parameter=target.parameter, method=target.method
            ),
            evidence=evidence,
        )
...
class ProductionTimeSQLiEngine(DetectionEngineProtocol):
    ...
    def _generate_time_payloads(self) -> list[tuple[str, float]]:
        ...
        return [
            ...
            (
                "' AND (SELECT 1 FROM (SELECT COUNT(*),CONCAT(version(),FLOOR(RAND(0)*2))x FROM information_schema.tables GROUP BY x)a) AND SLEEP(5)--",
                5.0,
            ),  # MySQL advanced
...
class ProductionSQLiModule(BaseFunctionModule):
    ...
    def __init__(self, config: SQLiConfig | None = None) -> None:
        ...
        super().__init__(
            ModuleName.FUNC_SQLI, config or SQLiConfig(), detection_engines
        )
