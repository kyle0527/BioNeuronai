...
這是 RLHF (Reinforcement Learning from Human Feedback) 在新聞分析的應用

工作流程：
1. 識別品種 (Symbol Extraction)
   - AI 分析新聞時必須明確標記目標幣種（BTC, ETH, SOL等）
   - 如果新聞沒寫，根據相關性標記

2. 發布預言 (The Prediction)
   - AI 給出分析：「這則新聞是利多，預計 BTC 在 4小時內 會上漲」
   - 將預測寫入 news_predictions 表，狀態設為 PENDING

3. 時間膠囊 (Time Capsule)
   - 系統設置排程任務，每小時掃描 PENDING 的預測
   - 當時間到達（例如 4 小時後），觸發驗證程序

4. 真相驗證 (Ground Truth Check)
   - 系統抓取該幣種現在的價格，對比發新聞時的價格
   - 判定：預測漲 + 實際漲 = 準確；預測漲 + 實際跌 = 錯誤

5. 權重修正 (Weight Update)
   - 如果判斷錯了，回頭修正 MarketKeywords 權重
   - 或降低該新聞來源的信賴度

6. 人類校準 (Human-in-the-Loop)
   - 關鍵預測可以標記為「需要人工確認」
   - 交易員可以給出反饋：「這個分析是對的/錯的」
   - 系統根據人類反饋調整模型
...
v4.0 更新 (2026-01-26):
- 簡化 RLHF：自動判斷準確度，不需要複雜人工評分
- 添加 PredictionScheduler：自動調度驗證任務
- 準確就是好，不準確就是壞，系統自動學習
"""
