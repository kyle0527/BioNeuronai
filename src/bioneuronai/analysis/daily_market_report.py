"""
📊 每日市場分析報告系統 (Daily Market Analysis Report)
==================================================
...
核心功能：
1. 市場環境掃描 - 全球市場、加密貨幣情緒
2. 新聞與情緒分析 - 整合 CryptoNewsAnalyzer 結果
3. 交易策略建議 - 依據市場狀況推薦最佳策略
4. 風險參數計算 - 資金管理與倉位建議
5. 交易對篩選 - 流動性與波動性匹配
...
class SOPAutomationSystem:
    ...
    def _import_modules(self):
        ...
        try:
            ...
            from .news_analyzer import CryptoNewsAnalyzer  # 同一資料夾
    ...
    async def _get_global_market_data(self) -> Optional[Dict]:
        ...
        複雜度降低策略：Extract Method 分離各數據收集模塊
        
        主要功能：
        1. 全球股市指數：美股、歐股、日股
        2. 加密貨幣情緒：恐慌貪婪指數
        3. 加密貨幣市場：Binance 24hr 數據
        """
        try:
            # 收集各模塊數據 (Extract Method)
            global_indices = await self._get_global_stock_indices()
            fng_data = await self._get_fear_greed_index()
            market_sentiment = await self._get_binance_market_sentiment()
            
            # 分析全球股市趨勢 (Extract Method)
            stock_analysis = self._analyze_global_stock_trends(global_indices)
            
            # 分析加密貨幣綜合情緒 (Extract Method)
    ...
    def _analyze_global_stock_trends(self, global_indices: Optional[Dict]) -> Dict[str, Any]:
        ...
        # 分析各區域市場
        us_markets = self._analyze_regional_market(global_indices, ['S&P 500', 'Dow Jones', 'NASDAQ'])
        european_markets = self._analyze_regional_market(global_indices, ['FTSE 100', 'DAX', 'CAC 40'])
        asian_markets = self._analyze_regional_market(global_indices, ['Nikkei 225', 'TOPIX'])
    ...
    def _analyze_regional_market(self, global_indices: Dict, market_names: List[str]) -> str:
        """分析區域市場趨勢"""
        changes = [
        ...
        return "BULLISH" if avg > 0.3 else ("BEARISH" if avg < -0.3 else "NEUTRAL")
    ...
    def _build_market_data_response(
        ...
        return {
            ...
            # 綜合評估
            'overall_sentiment': stock_analysis['global_stock_trend'] if global_indices else crypto_analysis['crypto_sentiment'],
            'sentiment_score': stock_analysis['global_stock_avg_change'] if global_indices else (crypto_analysis['crypto_sentiment_score'] - 50) / 50,
            'data_source': 'Yahoo Finance + Fear & Greed Index + Binance 24hr Ticker',
            'last_update': datetime.now().isoformat()
        }
    
    async def _get_global_stock_indices(self) -> Optional[Dict]:
        """
        獲取全球主要股市指數（美股、歐股、日股）
    ...
    async def _get_binance_market_sentiment(self) -> Optional[Dict]:
    ...
    async def _perform_ai_news_analysis(self) -> Optional[Dict]:
        ...
        try:
            ...
            # 讀取新聞分析器的產出結果（不重複執行分析）
            # 新聞分析器應該已經在背景執行並產出結果
            analysis = self.news_analyzer.analyze_news("BTCUSDT", hours=24)
            
            logger.info(f"📰 [OK] 讀取新聞分析結果：{analysis.total_articles}篇，情緒 {analysis.overall_sentiment}")
