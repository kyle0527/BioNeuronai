...
class BinanceFuturesConnector:
    ...
    def get_leverage_brackets(self, symbol: Optional[str] = None) -> Optional[List[Dict]]:
        """獲取交易對的槓桿分層標準 (Leverage Brackets)

        參數:
            symbol: 交易對符號，例如 "BTCUSDT"。若不傳入則獲取所有交易對的分層。

        返回:
            列表，每個元素代表一個交易對的階梯分層資訊。
            包含 "brackets" 陣列 (內含 bracket, initialMarginRate, maintenanceMarginRate, maxNotional 等)
        """
        params = {}
        if symbol:
            params["symbol"] = symbol

        # 注意: /fapi/v1/leverageBracket 需要簽名 (USER_DATA)
        result = self._make_request("GET", "/fapi/v1/leverageBracket", params, signed=True)
        if isinstance(result, list):
            return result
        return None
