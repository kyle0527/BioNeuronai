...
class MarketEvaluator:
    ...
    def identify_market_regime(self, ohlcv_data: np.ndarray) -> MarketRegime:
        ...
        return self._determine_regime(
            adx, close,
            float(sma_20), float(sma_50), float(trend_strength),
            float(vol_ratio), float(range_20),
        )
    ...
    def _apply_preference_adjustments(
        ...
        # 偏好簡單策略
        if 'prefer_simple' in preferences:
            if config.complexity.value == 'simple':
                adjustment += 0.05
    ...
    def filter_viable_strategies(
        ...
        for name, score in scores.items():
            ...
            # 複雜度檢查
            if not self._check_complexity_preference(config, preferences):
                continue
