...
class StrategyArena:
    ...
    def _create_strategy(self, candidate: StrategyCandidate) -> BaseStrategy:
        ...
        init_sig = inspect.signature(strategy_class)
        init_kwargs = {
            key: value
        ...
        if isinstance(strategy, TrendFollowingStrategy):
            strategy.ema_periods = {
                'fast': int(params.get('ema_fast') or strategy.ema_periods['fast']),
                'medium': int(params.get('ema_medium') or strategy.ema_periods['medium']),
                'slow': int(params.get('ema_slow') or strategy.ema_periods['slow']),
            }
            strategy.min_adx_for_trend = int(
                params.get('min_adx_for_trend') or strategy.min_adx_for_trend
            )
        elif isinstance(strategy, SwingTradingStrategy):
            strategy.swing_lookback = int(params.get('swing_lookback') or strategy.swing_lookback)
            strategy.rsi_period = int(params.get('rsi_period') or strategy.rsi_period)
            strategy.required_confirmations = int(
                params.get('required_confirmations') or strategy.required_confirmations
            )
        elif isinstance(strategy, MeanReversionStrategy):
            strategy.bb_period = int(params.get('bb_period') or strategy.bb_period)
            strategy.bb_std_dev = float(params.get('bb_std_dev') or strategy.bb_std_dev)
            strategy.z_lookback = int(params.get('z_lookback') or strategy.z_lookback)
        elif isinstance(strategy, BreakoutTradingStrategy):
            strategy.range_lookback = int(params.get('range_lookback') or strategy.range_lookback)
            strategy.breakout_threshold = float(
                params.get('breakout_threshold') or strategy.breakout_threshold
            )
            strategy.volume_threshold = float(
                params.get('volume_threshold') or strategy.volume_threshold
            )
        elif isinstance(strategy, DirectionChangeStrategy):
            strategy.min_consecutive_dc = int(
                params.get('min_consecutive_dc') or strategy.min_consecutive_dc
            )
