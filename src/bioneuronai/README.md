...
BioNeuronai 是一個 AI 驅動的加密貨幣期貨交易系統，以五層分層架構實現從市場數據接入到自動化交易的完整管線。系統整合了遺傳演算法策略進化、AI 推理、RLHF 新聞學習、多策略融合與系統化風控。

### 系統亮點
- ✅ **五層分層架構** — 職責分明、模組解耦
- ✅ **4 種基礎策略** — 趨勢 / 波段 / 均值回歸 / 突破
- ✅ **AI 策略融合** — 多策略動態加權
- ✅ **遺傳演算法進化** — 策略自適應進化（競技場 + 路由器 + 優化器）
...
| `trading_strategies.py` | 舊版策略接口（向後兼容） |
| `__init__.py` | 套件初始化 |
...
| 交易策略 | 4 種基礎 + AI 融合 |
| 進化組件 | 3 大組件 (Arena / Router / Optimizer) |
| 數據庫表 | 6 張 (SQLite) |
