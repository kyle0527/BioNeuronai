...
class RiskManager:
    ...
    def update_balance(self, balance: float) -> None:
        """更新交易帳戶餘額與峰值，供交易前回撤檢查使用。"""
        numeric_balance = float(balance)
        if not np.isfinite(numeric_balance) or numeric_balance < 0:
            raise ValueError(f"帳戶餘額必須是非負有限值: {balance}")
        if self.initial_balance == 0.0 and numeric_balance > 0:
            self.initial_balance = numeric_balance
        self.current_balance = numeric_balance
        self.peak_balance = max(self.peak_balance, numeric_balance)
    ...
    def check_can_trade(
        ...
        """依信心、餘額與目前回撤判斷是否允許建立新倉位。"""
        confidence = float(signal_confidence)
        balance = float(account_balance)
        if not np.isfinite(confidence) or not 0.0 <= confidence <= 1.0:
            return False, "交易信心必須介於 0 與 1"
        if not np.isfinite(balance) or balance <= 0:
            return False, "帳戶可用餘額不足"

        self.update_balance(balance)
        if confidence < 0.5:
            return False, f"交易信心不足: {confidence:.2%} < 50.00%"

        params = self.risk_parameters["MODERATE"]
        drawdown = (
            (self.peak_balance - self.current_balance) / self.peak_balance
            if self.peak_balance > 0
            else 0.0
        )
        if drawdown > params.max_drawdown_limit:
            return False, (
                f"目前回撤 {drawdown:.2%} 超過限制 "
                f"{params.max_drawdown_limit:.2%}"
            )
        return True, "風險檢查通過"

    def record_trade(self, trade_info: Dict[str, Any]) -> None:
        """記錄已由交易所或 paper connector 接受的訂單。"""
        record = dict(trade_info)
    ...
    def get_risk_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recorded_trades": len(self.trade_history),
            "active_alerts": len(
                [
                    alert
