...
7. `src/bioneuronai/planning/pretrade_automation.py`
...
| 計劃系統職責 | 舊 `trading_plan_system.py` 與 `plan_controller.py` 都曾做計劃編排 | 兩套流程長期分岔 | ✅ 已收斂：`trading_plan_system.py` 已移除，正式入口為 `planning/plan_controller.py` |

### 12.2 To-Be 架構（重規劃）

1. `analysis/news`：只負責分析與結構化結果。
2. `rag/services`：只負責入庫、查詢適配，不輸出交易流程類別。
3. `planning/pretrade_automation`：唯一交易前新聞判斷入口。
4. `planning/plan_controller`：唯一交易計劃編排入口。
...
4. D.4（已完成）：
- 舊 `trading_plan_system.py` 已移除，正式計劃入口統一為 `planning/plan_controller.py`。
