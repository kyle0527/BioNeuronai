...
第 4 層 (管理層)
  ↓
bioneuronai.planning/ ← 規劃 / pretrade / 市場分析
bioneuronai.trading/  ← 訂單 / 帳戶 / 持倉 / 資金狀態
...
schemas/  ◄───────────────────────────────────────┐
  ...
  └──► bioneuronai.backtest/  ◄────┴─────┴────────┘
         ...
         └── 使用 bioneuronai.trading.virtual_account
...
| [bioneuronai/trading/virtual_account.py](../src/bioneuronai/trading/virtual_account.py) | 虛擬帳戶 | 模擬帳戶狀態、訂單撮合、資金變動 |
...
#### 2.6 bioneuronai.planning/ - 交易規劃層
...
#### 2.7 bioneuronai.trading/ - 訂單與帳戶事實層

**作用**: 收斂訂單、帳戶、持倉、資金與成交事實來源

**目前檔案**:
- `virtual_account.py` - replay / mock execution 所使用的虛擬帳戶
