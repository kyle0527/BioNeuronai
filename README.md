...
| 特性 | 描述 | 狀態 |
|------|------|------|
| 🧠 **AI 神經網路** | 111.6M 參數 TinyLLM（GPT 雙模態），交易訊號 + 自然語言對話共用一份權重 | ✅ 完成 |
...
│
├── 📁 model/                     # AI 模型權重
│   ├── my_100m_model.pth         # TinyLLM 111.6M（InferenceEngine + ChatEngine 共用）
│   └── tokenizer/
...
│                       ChatEngine (對話助理)                      │
├─────────────────────────────────────────────────────────────────┤
│  TinyLLM.generate() ← 同一份 my_100m_model.pth                  │
...
### 最新里程碑：v2.2 TinyLLM 雙模態整合 (2026-04)
...
| `model/` | [model/README.md](model/README.md) | TinyLLM 模型權重（my_100m_model.pth） |
