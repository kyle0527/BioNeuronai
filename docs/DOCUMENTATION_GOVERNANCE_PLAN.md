...
`v2.2` 目前只代表 roadmap、雲端訓練與 Meta-Learner 過渡工作，不應在正式操作文件中寫成已發布主版本。
...
- `v2.2` 只能用於 roadmap、過渡期或雲端訓練工作，必須明確標示尚未完全發布
...
3. `V2.2_ROADMAP_AND_SPEC.md` 只能描述 roadmap 與過渡狀態，不得把未完成的 UI 點擊驗收、Docker image 複驗或雲端訓練 job 寫成已完成。
...
- `v2.2` 只能出現在 roadmap 或明確標示的過渡期文件中
