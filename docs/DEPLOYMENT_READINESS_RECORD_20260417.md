...
- `logs/` 目錄需存在，因為 `docker-compose.yml` 預設 bind `${LOGS_DIR:-./logs}`。
...
| Docker daemon | 未通過 | `docker ps` 無法連線到 Docker Desktop Linux engine |

更新後判斷：內部 staging 距離縮短，但仍需要 Docker daemon、pretrade/backtest/chat 端到端驗收與 secrets 處理。
