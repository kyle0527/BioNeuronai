...
**程式碼完整，需要 torch 和模型權重。**

- 模型：111.2M 參數 MLP（`model/my_100m_model.pth`，445MB，Git LFS 管理）
- 權重取得：本地執行 `git lfs pull` 即可下載完整權重
...
### 13. Docker 容器化部署（v4.4 新增）

**完整實作，多階段建構 + service profiles。**
...
- **Dockerfile**：builder（ta-lib + deps）→ runtime（精簡映像，非 root）
- **docker-compose.yml**：7 個 service profiles（status/news/pretrade/plan/backtest/simulate/trade）
- **PYTHONPATH** 已正確設定為 `/app/src`
...
| 端點 | 類型 | 說明 |
|------|------|------|
| `/api/v1/status` | 同步 | 系統健康狀態 |
| `/api/v1/plan` | 同步 | 10 步驟交易計劃 |
| `/api/v1/news` | 同步 | 新聞情緒分析 |
| `/api/v1/pretrade` | 同步 | 交易前 5 步驟檢查 |
| `/api/v1/backtest` | 背景工作 | 歷史回測 |
| `/api/v1/simulate` | 背景工作 | 紙交易模擬 |
| `/api/v1/jobs/{id}` | 查詢 | 背景工作進度 |
| `/api/v1/trade/start` | 控制 | 啟動交易 |
| `/api/v1/trade/stop` | 控制 | 停止交易 |

---

### 15. RAG TTL 快取（v4.4 修復）

**已實作，in-memory TTL 快取（預設 5 分鐘）。**

- `UnifiedRetriever` 重複查詢命中快取時跳過向量搜尋
- `cache_hit` flag 現在正確傳遞至 `monitor.log_retrieval()`
- 此前 TODO：`cache_hit=False` 硬編碼問題已解決
...
| 模組 | 檔案 | 行數 | 完成度 |
|------|------|------|--------|
| REST API 伺服器 | `api/app.py` | 448 | ✅ 100% |
| REST API 模型 | `api/models.py` | 78 | ✅ 100% |
