...
| `plan` | ⚠️ | ⚠️ | 部分可用，步驟 5/6/9/10 回傳模擬數據 |
| `backtest` | ❌ | ⚠️ | 需要本地歷史數據檔案（目錄不存在） |
...
| **Step 5：策略性能評估** | ❌ 模擬 | 硬編碼勝率（65%/58%/62%），非真實回測 |
| **Step 6：策略選擇權重** | ❌ 模擬 | 硬編碼權重 0.7，非動態計算 |
| Step 7：風險參數計算 | ✅ 完整 | 根據波動率動態調整 |
| Step 8：資金管理規劃 | ✅ 完整 | Kelly 倉位真實計算 |
| **Step 9：交易對篩選** | ❌ 模擬 | 靜態回傳 BTC/ETH/SOL，未查詢 API |
| **Step 10：執行計劃監控** | ❌ 模擬 | 回傳假的 WebSocket 就緒狀態 |

**問題根源**：4 個 SIMPLIFIED 步驟均標有「待未來整合」的 TODO 註解，目前直接回傳硬編碼字典。
...
- 歷史數據目錄：❌ `data_downloads/binance_historical/` **不存在**
...
目前回傳：
```python
{"trend_following": {"win_rate": 0.65, "sharpe": 1.8}}  # 假數據
```

應改為：從 BacktestEngine 執行真實策略回測，計算實際績效。

---

### 3. 動態交易對篩選（Step 9）

**靜態列表，需要連接 Binance API 即時篩選。**

目前靜態回傳 BTCUSDT / ETHUSDT / SOLUSDT，未考慮：
- 24h 成交量即時排名
- 當前波動率分布
- 幣對間相關性過濾
...
**模擬就緒狀態，WebSocket 未真正建立。**

目前回傳 `"websocket_ready": True`，但實際上未訂閱任何數據流，無法發送價格警報或倉位警報。

---

### 5. 新聞情感整合（market_analyzer.py）
