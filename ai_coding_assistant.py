            ...
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(target, ast.Name):
                        ...
                        if any(keyword in var_name for keyword in ['password', 'passwd', 'pwd', 'secret', 'key', 'token']):
...
class AICodeAssistant:
    ...
    def save_report(self, report: ImprovementReport) -> Path:
        """保存報告"""
        report_file = self.reports_dir / f"analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
