...
from bioneuronai.improved_core import CuriositDrivenNet, ImprovedBioNeuron
...
def anomaly_detection_demo():
    ...
    detector = ImprovedBioNeuron(
        num_inputs=3, threshold=0.4, memory_len=10, adaptive_threshold=True, seed=42
    )
    ...
    for data, true_label in test_data:
        ...
        print(
            f"{data_str}\t{output:.3f}\t{novelty:.3f}\t{predicted}\t{true_label} {status}"
        )
...
def adaptive_learning_demo():
    ...
    for env_name, data_generator in environments:
        ...
        print(
            f"  {env_name} 適應狀態: {adaptation_status} (好奇心: {final_curiosity:.3f})"
        )
...
def real_time_monitoring_demo():
    ...
    while time.time() - start_time < 3:  # 運行3秒作為演示
        ...
        print(
            f"{step:2d}s\t{cpu_usage:.2f}\t{memory_usage:.2f}\t{network_traffic:.2f}\t{alert_str}"
        )
...
def pattern_discovery_demo():
    ...
    discoverer = ImprovedBioNeuron(
        num_inputs=4, threshold=0.3, memory_len=20, adaptive_threshold=True, seed=42
    )
    ...
    print(
        f"學習統計: 激活率={stats['activation_rate']:.2f}, "
